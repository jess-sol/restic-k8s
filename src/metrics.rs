...
impl Default for Metrics {
    fn default() -> Self {
        ...
        let failures = IntCounterVec::new(
            opts!("doc_controller_reconciliation_errors_total", "reconciliation errors",),
            &["instance", "error"],
        )
        .unwrap();
        let reconciliations =
            IntCounter::new("doc_controller_reconciliations_total", "reconciliations").unwrap();
        Metrics { reconciliations, failures, reconcile_duration }
    }
}
...
impl Metrics {
    ...
    pub fn count_and_measure(&self) -> ReconcileMeasurer {
        ...
        ReconcileMeasurer { start: Instant::now(), metric: self.reconcile_duration.clone() }
    }
}
